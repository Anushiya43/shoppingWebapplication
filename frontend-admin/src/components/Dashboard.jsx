...
const Dashboard = () => {
  ...
  // 5. UNIFIED RENDER (NO EARLY RETURNS)
  return (
    <div className="min-h-screen bg-bg-slate relative overflow-hidden">
      {/* Background Decor (Common to all states) */}
      <div className="absolute top-0 left-0 w-96 h-96 bg-accent-blue/10 rounded-full blur-[100px] -ml-48 -mt-48 pointer-events-none"></div>
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-accent-cyan/10 rounded-full blur-[100px] -mr-48 -mb-48 pointer-events-none"></div>

      {loading ? (
        /* Case 1: Synchronizing session */
      ...
      ) : !user && error !== 'admin_only' ? (
        /* Case 2: Not logged in */
        <div className="min-h-screen flex items-center justify-center p-4 relative z-10">
          <LoginModal isOpen={isLoginModalOpen} onClose={() => setLoginModalOpen(false)} />
          <div className="max-w-md w-full bg-white/80 backdrop-blur-xl rounded-[3rem] p-10 shadow-2xl border border-white/50 text-center">
